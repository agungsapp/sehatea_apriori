...
final class KategoriTable extends PowerGridComponent
{
    ...
    public string $tableName = 'kategori-table-lpg4fv-table';
    ...
    public function confirmBulkDelete()
    {
        ...
        if (empty($this->checkboxValues)) {
            $this->alert('info', 'Tidak ada kategori yang dipilih.', [
    ...
    public function bulkDeleteConfirmed()
    {
        if ($this->checkboxValues) {
            ...
            Kategori::destroy($this->checkboxValues);
    ...
    {
        return Kategori::query()->orderBy('id', 'desc');
    }
    ...
    public function actions(Kategori $row): array
    {
        return [
            Button::add('edit')
                ...
                ->dispatch('edit-kategori', ['id' => $row->id]),
            Button::add('delete')
                ->slot("<i class='bx bxs-trash'></i>")
                ->id()
                ->class('btn btn-danger')
                ->dispatch('hapus-kategori', ['id' => $row->id])
        ];
    }
