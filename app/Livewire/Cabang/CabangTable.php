...
final class CabangTable extends PowerGridComponent
{
    use LivewireAlert;
    public string $tableName = 'cabang-table-nkrpnu-table';
    ...
    public function bulkDeleteConfirmed()
    {
        if ($this->checkboxValues) {
            ...
            Cabang::destroy($this->checkboxValues);
    ...
    {
        return Cabang::query()->orderBy('id', 'desc');
    }
    ...
    public function actions(Cabang $row): array
    {
        return [
            Button::add('edit')
                ...
                ->dispatch('edit-cabang', ['id' => $row->id]),
            Button::add('delete')
                ->slot("<i class='bx bxs-trash'></i>")
                ->id()
                ->class('btn btn-danger')
                ->dispatch('delete-cabang', ['id' => $row->id])
        ];
    }
